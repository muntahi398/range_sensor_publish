...
class VL6180X:
    ...
    def default_settings(self):
        ...
        # http://www.st.com/st-web-ui/static/active/en/resource/technical/document/application_note/DM00122600.pdf
        # Set default ranging inter-measurement period to 100ms  ## mnk x09 to xo2
        self.set_register(self.__VL6180X_SYSRANGE_INTERMEASUREMENT_PERIOD, 0x02)
        # Set default ALS inter-measurement period to 100ms
        self.set_register(self.__VL6180X_SYSALS_INTERMEASUREMENT_PERIOD, 0x31)
        # Configures interrupt on 'New Sample Ready threshold event'  ## mnk 24 to 4
        self.set_register(self.__VL6180X_SYSTEM_INTERRUPT_CONFIG_GPIO, 0x4)
    ...
    def get_distance(self):
        ...
        return distance
    def get_distance_intrpt_s1(self):
        # Start Single shot mode
        self.set_register(self.__VL6180X_SYSRANGE_START, 0x01)
        time.sleep(0.015)
#        if self.debug:
#            print("Range status: %x", self.get_register(self.__VL6180X_RESULT_RANGE_STATUS) & 0xF1)
    ...
    def get_distance_intrpt_s2(self):
        # Start Single shot mode
        self.set_register(self.__VL6180X_SYSRANGE_START, 0x01)
#        time.sleep(0.010)
#        if self.debug:
#            print("Range status: %x", self.get_register(self.__VL6180X_RESULT_RANGE_STATUS) & 0xF1)
        if GPIO.input(19):
            ...
            self.set_register(self.__VL6180X_SYSTEM_INTERRUPT_CLEAR, 0x07)
            return distance
    def get_distance_intrpt_s3(self):
        # Start Single shot mode
        self.set_register(self.__VL6180X_SYSRANGE_START, 0x01)
        time.sleep(0.015)
#        if self.debug:
#            print("Range status: %x", self.get_register(self.__VL618$
        if GPIO.input(26):
            distance = self.get_register(self.__VL6180X_RESULT_RANGE_VAL)
            self.set_register(self.__VL6180X_SYSTEM_INTERRUPT_CLEAR, 0x07)
            return distance
    ...
    def range_start_req(self):
        # Start Single shot mode
        self.set_register(self.__VL6180X_SYSRANGE_START, 0x01)
    def range_result_acquire(self): # ->(int):
        distance = self.get_register(self.__VL6180X_RESULT_RANGE_VAL)
        self.set_register(self.__VL6180X_SYSTEM_INTERRUPT_CLEAR, 0x07)
        return distance
    def range_result_acquire_cont(self): # ->(int):
        distance = self.get_register(self.__VL6180X_RESULT_RANGE_VAL)
        self.set_register(self.__VL6180X_SYSTEM_INTERRUPT_CLEAR, 0x07)
        self.set_register(self.__VL6180X_SYSRANGE_START, 0x01)
        return distance
    def range_start_req_cont(self):
        # Start Single shot mode
        self.set_register(self.__VL6180X_SYSRANGE_START, 0x02)
        self.set_register(self.__VL6180X_SYSRANGE_START, 0x01)
    def set_continuous_range_start_req(self):
    ...
    def range_result_acquire(self): #-> (int):    
        distance = self.get_register(self.__VL6180X_RESULT_RANGE_VAL)
        self.set_register(self.__VL6180X_SYSTEM_INTERRUPT_CLEAR, 0x01)  # was 7, but that said 0x01
        return distance
        
    def set_continuous_stop_req(self):
        self.set_register(self.__VL6180X_SYSRANGE_START, 0x01)
        self.set_register(self.__VL6180X_INTERLEAVED_MODE_ENABLE,0)
    def interrupt_clear(self):
        # Start Single shot mode
        self.set_register(self.__VL6180X_SYSTEM_INTERRUPT_CLEAR,0x07)
    def time_sensr(self):
        time =  self.get_register_16bit(self.__VL6180X_IDENTIFICATION_TIME)
        return time
